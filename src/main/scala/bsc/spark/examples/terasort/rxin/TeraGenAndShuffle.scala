...
//From https://github.com/apache/spark/pull/1242/commits
//https://github.com/rxin/spark/tree/4b88e3eb548ee9aaf6d864e8b4389dccc6d462eb/examples/src/main/scala/org/apache/spark/examples/terasort
...
import bsc.spark.examples.terasort.Unsigned16
import bsc.spark.examples.terasort.Random16
  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 3) {
      ...
      println("DRIVER_MEMORY=[mem] bin/run-example org.apache.spark.examples.terasort.TeraSort " +
        "[input-size] [input-parts] [output-parts] <path-to-tmp-local-dir>")
      println(" ")
      println("example:")
    ...
    val conf = new SparkConf().setAppName(s"TeraSort ($size)");
	
	if (args.length > 3) {
	  		conf.set("spark.local.dir", args(3));
			println("spark.local.dir set to " + args(3)); //e.g. $TMPDIR in MareNostrum
	}	
    ...
    //val output = new ShuffledRDD[Array[Byte], Array[Byte], Array[Byte], RecordWrapper](dataset, partitioner)
