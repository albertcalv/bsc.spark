...
abstract class KVDataTest2(sc: SparkContext, dataType: String = "string") extends PerfTest {
  // TODO (pwendell):
  ...
  val stringOptions = Seq(PERSISTENCE_TYPE, STORAGE_LOCATION, PATH)
  val booleanOptions = Seq(WAIT_FOR_EXIT, HASH_RECORDS)
  val options = longOptions ++ intOptions ++ stringOptions  ++ booleanOptions
  
  //RUBEN: This parser is ready to parse the arguments 
  val parser = new OptionParser()
  //RUBEN: This optionSet will keep the values of the arguments (initialized in the initialize method)
  ...
  override def initialize(args: Array[String]) = {
	//RUBEN: Here we parse the arguments
  ...
  override def createInputData() = {
	...
	val path: String = optionSet.valueOf(PATH._1).asInstanceOf[String]		
	...
	//rdd.map{case (k, v) => "%s\t%s".format(k, v)}.saveAsTextFile(path, classOf[DefaultCodec]) //compressed
	rdd.map{case (k, v) => "%s\t%s".format(k, v)}.saveAsTextFile(path)
    println("DATA SAVED.")
    //RUBEN  
...
class SortByKeyDisk(sc: SparkContext) extends KVDataTest2(sc) {  
  ...
  override def createInputData() {
    ...
    rdd = dataLines.map(_.split("\t")).map(x => (x(0), x(1)))
    println("DATA LOADED.")        
  }
  //RUBEN
}

class GenKeyToDisk(sc: SparkContext) extends KVDataTest2(sc) {
  override def runTest(rdd: RDD[_], reduceTasks: Int) {
    //Do nothing
  }
